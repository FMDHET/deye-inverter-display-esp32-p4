...
## Nachtrag 7 (10. September): der Hauptbildschirm sagt, was er nicht weiß

Ein Bildschirm, der eine Zahl zeigt, behauptet sie zu kennen. Drei Stellen taten das zu Unrecht — alle drei jetzt behoben, und keine davon ändert einen einzigen Wert, der zum Wechselrichter geht.

| Was | Wie |
| --- | --- |
| **Fehlt ein Wechselrichter, stand da 0,0 kW.** Der Fund vom Vortag: ein schweigendes Gerät fällt aus dem Energiemodell (richtig so), PV fällt dann auf die eigenen Solar-Eingänge des Deye zurück — die es hier nicht gibt — und der Hausverbrauch, eine Summe, kommt zu niedrig heraus. Nichts sagte ein Wort | Eine **Hinweiszeile unten mitte**, versteckt solange alles stimmt, mit vier Fällen nach Dringlichkeit: Zähler-Bus stumm (samt dem Rat, das Gerät stromlos zu machen — der Fall vom 8.9., den 40 Warmstarts nicht lösten), Emulation stumm nach abgelaufener Brücke, Phasenmanipulation aktiv mit Restzeit, und „*n* von *m* Geräten antworten nicht — Werte unvollständig" |
| **Phasenmanipulation war unsichtbar und endlos** — das einzige Werkzeug, das dem Deye absichtlich falsche Zahlen schickt | Steht jetzt auf dem Hauptbildschirm mit Restzeit, läuft nach 2 h ab (`MB_MANIP_MAX_S`, dieselbe Begründung und Dauer wie `DEYE_FORCE_MAX_S`) und wird bei **jedem Start abgeschaltet**: sie lebt nur im Display, ein unbeaufsichtigter Neustart darf nicht stillschweigend weiterlügen. Werte bleiben erhalten, nur der Hauptschalter fällt. Der Ablauf wird in `do_slave()` geprüft und nur im RAM gelöscht — ein Flash-Schreibvorgang würde dort eine Antwort verzögern, auf die der Deye wartet |
| **Nach einem Dateisystem-Update behauptete das Build-Etikett einen Versatz, den es nicht gab** (`FW #255 / FS #249`, während `/ota` schon 255 meldete) | Das Etikett wurde einmal beim Start gesetzt. `ota_fs_handler()` sagt es der Oberfläche jetzt direkt nach dem Wiedereinhängen |

**Am Gerät gesehen** (diesmal per OTA — das USB-Kabel hängt nicht mehr dran, wofür `/log` und der Coredump von gestern gerade rechtzeitig kamen): direkt nach dem Neustart „3 von 4 Geräten antworten nicht" mit „--" in den Kreisen statt erfundener Nullen; eine Minute später alle vier verbunden und die Zeile verschwunden; das Etikett grün statt falschem Alarm. Der Manipulations-Hinweis ließ sich **wirkungsfrei** prüfen: Hauptschalter an, alle drei Phasen auf „unverändert" — die Zeile erscheint samt „noch 120 min", der Deye bekommt dabei exakt dieselben Werte wie vorher. Danach wieder aus.

Nicht am Gerät gesehen: der Hinweis für den tauben Zähler-Bus. Den gibt es nur mit echter Hardware-Störung — die Logik dahinter ist aber getestet (siehe unten).
